...
- 首页：学习方向、最近文章、精选项目（优先展示已验证项目）
- 关于页：定位、方向、最近进展、站点使用说明与订阅入口
- 学习记录列表与 5 篇 MDX 文章详情
- 项目列表与 4 个项目详情
- 自定义 404 页面
...
1. 自建 AI 聚合站之前的 MVP 设计笔记（2026-09-11）
2. 用 CubeMX 与 Proteus 完成 STM32F103C8T6 的第一次仿真（2026-09-10）
...
1. 技术成长博客（本站）— 已实现并验证
   - 2026-09-13 完成生产验收（HTTPS、404、RSS、sitemap、Lighthouse）
2. STM32F103C8T6 串口命令控制台工程 — 主机端已验证，硬件待验证
   - CubeIDE 零警告编译与 Proteus 仿真已有记录；上板行为未验证
...
生产验收之后新增两篇学习记录与两个项目卡片，扩充关于页与首页精选逻辑，并同步调整了测试：

- `tests/site.spec.ts` 页面清单加入 4 个新页面，博客排序与 RSS 条目数断言更新为 5 篇。
- Playwright 控制台错误断言过滤 `static.cloudflareinsights.com/beacon.min.js`：该脚本由 Cloudflare 自动注入，能否加载取决于客户端网络，与站点源码无关，不应作为回归失败条件。
